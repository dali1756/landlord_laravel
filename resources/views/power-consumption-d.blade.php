...
<section id="main" class="wrapper">
	...
	<div class="container">
		<ul class='nav nav-tabs nav-border'>
			<li class='nav-item'> 
				<a class='nav-link card card-border active' id='tab_day' href="power-consumption-d" role='tab' aria-selected='false'>
					<span class='h5 mb-0 font-weight-bold '>每日用電</span>
				</a>    
			</li>
			<li class='nav-item'>
				<a class='nav-link card card-border' id='tab_month' href="power-consumption-m" role='button' aria-selected='false'>
	...
	<div class="container">
		<form id='mform2' action="power-consumption-d" method="post" class='col-12'>
			@csrf
			...
			<div class="form-group row select-mar3"  id='select_day' >
				<label class="col-sm-2 col-form-label label-right">輸入日期</label>
				<div class="col-sm-9"> 
					<input class="form-control form-control2" type="date" placeholder="yyyy-mm-dd" size="20" id="day" name='day' value="{{ $day }}" required>
				</div>
			</div>
			<br>
			<div class='col-12'>
		...
		@if(!empty($result))
			<h1 class="jumbotron-heading text-center h1-mar">查詢結果</h1>
				<div class=" text-right ">
					<form id='mform1' action="power_consumption_d_excel" method="post" class='col-12'>
					@csrf
